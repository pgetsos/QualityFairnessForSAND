...
from adaptation import basic_dash, basic_dash2, weighted_dash, netflix_dash, sand
...
def download_segment(segment_url, dash_folder):
    ...
    try:
        connection = urlopen(segment_url)
    except HTTPError as error:
...
def start_playback_smart(dp_object, domain, playback_type=None, download=False, video_segment_duration=None):
    ...
    qs_list = defaultdict(defaultdict) # No idea what this is tbh
    ...
    for bitrate in dp_object.video:
        ...
        segment_quality_scores = dp_object.video[bitrate].squality_list
        #print "media urls"
        #print media_urls
        ...
        for segment_count, quality_score in enumerate(segment_quality_scores, dp_object.video[bitrate].start):
            qs_list[segment_count][bitrate] = quality_score

    quality_score_bitrates = dp_object.quality_list
    ...
    # Find client's IP
    ip = "192.168.1.1" # Change to expected client's one just in case
    if playback_type.upper() == "SAND":
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        try:
            s.connect(('10.0.0.7', 8080))
            ip = s.getsockname()[0]
        except OSError:
    ...
    for segment_number, segment in enumerate(dp_list, dp_object.video[current_bitrate].start):
        ...
        if playback_type.upper() == "SAND":
            # segment_quality = qs_list[segment_number][current_bitrate]
            recent_segment_qualities.append(quality_score)
            ...
            config_dash.LOG.debug(
                "SAND-DASH: Selected {} for the segment {}".format(current_bitrate,
                                                                    segment_number + 1))
        elif segment_number == dp_object.video[bitrate].start:
            current_bitrate = bitrates[0]
        else:
        ...
        #print "segment url"
        #print segment_url
        config_dash.LOG.debug("{}: Segment URL = {}".format(playback_type.upper(), segment_url))
        if delay:
            delay_start = time.time()
            config_dash.LOG.debug("SLEEPING for {}seconds ".format(delay*segment_duration))
        ...
        config_dash.LOG.debug("{} : The total downloaded = {}, segment_size = {}, segment_number = {}".format(
            playback_type.upper(),
...
def main():
    ...
    configure_log_file(playback_type=PLAYBACK.lower(), client=CLIENT.lower(), exp=EXP.lower())
    ...
    elif "sand" in PLAYBACK.lower():
        config_dash.LOG.critical("Started SAND-DASH Playback")
        start_playback_smart(dp_object, domain, "SAND", DOWNLOAD, video_segment_duration)
